...
class MyPytorchModel(nn.Module):
    ...
    def __init__(self, hparams):
        ...
        a1=nn.Linear(hparams["input_size"],hparams["n_hidden"])
        a_last=nn.Linear(hparams["n_hidden"],hparams["num_classes"])

        if hparams["layer_norm"]:
            d1=nn.LayerNorm(hparams["input_size"])
            l1=nn.Sequential(d1,a1,nn.GELU())
            d_hidden=nn.LayerNorm(hparams["n_hidden"])
            l_last=nn.Sequential(d_hidden,a_last,nn.Softmax())
        else:
            l1=nn.Sequential(a1,nn.GELU())
            l_last=nn.Sequential(a_last,nn.Softmax())
        buffer=[]
        for _ in range(hparams["num_layers"]-2):
            a_hidden=nn.Linear(hparams["n_hidden"],hparams["n_hidden"])
            if hparams["layer_norm"]:
                d_hidden=nn.LayerNorm(hparams["n_hidden"])
                l_hidden=nn.Sequential(d_hidden,a_hidden,nn.GELU())
            else:
                b_hidden=nn.BatchNorm1d(hparams["n_hidden"])
                l_hidden=nn.Sequential(a_hidden,b_hidden,nn.GELU())
            buffer.append(l_hidden)
        self.model=nn.Sequential(l1,*buffer,l_last)
