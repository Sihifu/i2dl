...
class Classifier(Network):
    ...
    def backward(self, dout):
        ...
        X=self.cache[0]
        """
        a=(dout*(self.sigmoid(z)*(1-self.sigmoid(z))))[:,np.newaxis,:]
        b=X[:,:,np.newaxis]
        # caclulate the gradient of every single input (batch_size,H,W)
        dW_multi=a*b
        dW=np.sum(dW_multi,axis=0)
        """

        dW=X.T@(dout*(self.sigmoid(z)*(1-self.sigmoid(z))))
